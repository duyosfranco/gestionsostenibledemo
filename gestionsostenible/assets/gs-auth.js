(function(global){
  ...
  const PROVISIONER_APP_NAME = 'gs-admin-provisioner';
  let provisionerAuth = null;
  const recaptchaCache = new Map();
  const organizationCache = new Map();

  function getProvisionerAuth(){
    if(provisionerAuth){ return provisionerAuth; }
    let provisionerApp = null;
    try{
      provisionerApp = firebase.app(PROVISIONER_APP_NAME);
    }catch(err){
      try{
        provisionerApp = firebase.initializeApp(firebaseConfig, PROVISIONER_APP_NAME);
      }catch(initErr){
        console.error('No se pudo inicializar la app auxiliar para registrar usuarios', initErr);
        throw initErr;
      }
    }
    provisionerAuth = provisionerApp.auth();
    try{
      const Persistence = firebase.auth && firebase.auth.Auth && firebase.auth.Auth.Persistence
        ? firebase.auth.Auth.Persistence
        : null;
      if(Persistence && typeof provisionerAuth.setPersistence === 'function'){
        provisionerAuth.setPersistence(Persistence.NONE).catch(()=>{/* sin persistencia */});
      }
    }catch(err){
      console.warn('No se pudo ajustar la persistencia del registrador', err);
    }
    return provisionerAuth;
  }
  ...
  async function adminCreateUser(options){
    ...
    const adminAuth = getProvisionerAuth();
    const credential = await adminAuth.createUserWithEmailAndPassword(email, password);
    ...
    if(displayName){
      try{ await user.updateProfile({ displayName }); }
      catch(err){ console.warn('No se pudo actualizar el nombre del nuevo usuario', err); }
    }
    ...
    if(user && typeof user.sendEmailVerification === 'function'){
      try{
        ...
        console.warn('No se pudo enviar el correo de verificación inicial', err);
        welcomeEmailError = err;
      }
    }
    if(!welcomeEmailSent && typeof adminAuth.sendPasswordResetEmail === 'function'){
      try{
        await adminAuth.sendPasswordResetEmail(email);
        welcomeEmailSent = true;
      }catch(err){
        console.warn('No se pudo enviar el correo de restablecimiento como alternativa', err);
        if(!welcomeEmailError){ welcomeEmailError = err; }
      }
    }
    const snapshot = {
      uid: user.uid,
      ...
      welcomeEmailSent,
      welcomeEmailError: welcomeEmailError ? (welcomeEmailError.message || String(welcomeEmailError)) : null
    };
    if(db){
      try{
        ...
        await db.collection('usuarios').doc(user.uid).set(profilePayload, { merge: true });
      }catch(err){
    ...
    logAuditEvent('account.provisioned', {
      ...
      targetRole: requestedRole,
      organizationId
    }, { uid: user.uid, email, displayName: snapshot.displayName }).catch(()=>{});
    return snapshot;
  }
  ...
  async function adminDeleteUser(options){
    ...
    if(typeof fetch !== 'function'){
      return { success: false, authDeleted: false, error: 'El entorno no admite solicitudes de red.' };
    }
    const user = auth.currentUser;
    if(!user){
      return { success: false, authDeleted: false, error: 'No hay una sesión administrativa activa.' };
    }
    let idToken = null;
    try{
      idToken = await user.getIdToken(true);
    }catch(err){
      return { success: false, authDeleted: false, error: err && err.message ? err.message : 'No se pudo obtener un token de autenticación.' };
    }
    const endpoint = `https://identitytoolkit.googleapis.com/v1/projects/${firebaseConfig.projectId}/accounts:delete?key=${firebaseConfig.apiKey}`;
    const payload = { idToken };
    if(uid){ payload.localId = uid; }
    if(email && !uid){ payload.email = email; }
    try{
      ...
      if(!response.ok){
        let message = response.statusText || 'No se pudo eliminar la cuenta.';
        try{
          const data = await response.json();
          if(data && data.error && data.error.message){
            message = data.error.message;
          }
        }catch(parseErr){ /* sin cuerpo legible */ }
        return { success: false, authDeleted: false, error: message };
      }
